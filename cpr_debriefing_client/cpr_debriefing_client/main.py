...
def run_pipeline(args):
    ...
    nlp_events, comm_metrics = nlp.process(
        leader_segments=leader_segs,
        ceiling_segments=ceiling_segs,
    )
    ...
    from data.schemas.event_schema import UnifiedTimeline
    ...
    try:
        ...
        findings = raw_findings  # list[dict] from new engine
        logger.info(f"  {len(findings)} findings (via acls_engine.ACLSEngine)")
        for f in findings:
            sev = f.get("severity", "INFO")
            logger.info(f"    [{sev.upper()}] {f.get('rule_id', '')} | {f.get('deviation_message', '')}")
    except ImportError:
