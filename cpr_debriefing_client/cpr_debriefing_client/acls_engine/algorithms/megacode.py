...
class MegacodeAlgorithm(BaseAlgorithm):
    ...
    def __init__(self, rules: dict, sub_type: str = "brady_to_arrest"):
        ...
        self.sub_type = sub_type or "brady_to_arrest"  # Guard against None from classifier
        ...
        # ── Unpack bundled rulesets (from engine.py) or use single dict ────────────
        # engine.py passes {"cardiac_arrest": ..., "bradycardia": ..., "tachyarrhythmia": ...}
        # Legacy / direct calls pass a single flat rules dict — fall back gracefully.
        if "cardiac_arrest" in rules and "bradycardia" in rules:
            arrest_rules = rules["cardiac_arrest"]
            brady_rules  = rules["bradycardia"]
            tachy_rules  = rules["tachyarrhythmia"]
        else:
            arrest_rules = brady_rules = tachy_rules = rules

        # ── Sub-algorithm processors ───────────────────────────────────────
        # Phase 1 processor — correct ruleset per sub_type
        if self.sub_type == "brady_to_arrest":
            self._phase1_processor = BradycardiaAlgorithm(brady_rules)
        else:
            self._phase1_processor = TachyarrhythmiaAlgorithm(tachy_rules)

        # Phase 2 + 3 share one cardiac arrest processor
        self._arrest_processor = CardiacArrestAlgorithm(arrest_rules, sub_type="unknown")
    ...
    def _transition_to_phase2(self, event: dict) -> None:
        ...
        sub_label = (self.sub_type or "initial phase").replace("_", " ")
        self._emit(
            rule_id="R_MEGA_001", severity="INFO",
            message=f"Phase transition: {sub_label} -> Cardiac Arrest (VF/pVT) at {t}s",
            timestamp_sec=t,
