...
from analysis.scoring.completeness_flags import get_completeness_flag, get_completeness_note
from analysis.scoring.confidence import CICalculator
from analysis.scoring.domain_scorers.base_scorer import BaseDomainScorer
from analysis.scoring.schemas.event_schema import (
    EventType,
    FindingRecord,
    Severity,
    TranscriptSegment,
...
from analysis.scoring.score_models import DomainScore, SubSignalScore
...
def _estimate_ccf_from_timeline(timeline: UnifiedTimeline) -> float:
    ...
    """
    arrest_start = getattr(timeline, "arrest_start_ms", None)
    if arrest_start is None:
        arrest_ev = next((e for e in timeline.events if getattr(e, "event_type", None) in [EventType.ARREST_RECOGNIZED, EventType.CPR_INITIATED, "arrest_recognized", "cpr_initiated"]), None)
        arrest_start = getattr(arrest_ev, "timestamp_ms", 0) if arrest_ev else 0

    arrest_end = getattr(timeline, "rosc_ms", None)
    if arrest_end is None:
        rosc_ev = next((e for e in timeline.events if getattr(e, "event_type", None) in [EventType.ROSC_ACHIEVED, "rosc_achieved"]), None)
        arrest_end = getattr(rosc_ev, "timestamp_ms", None) if rosc_ev else getattr(timeline, "duration_ms", getattr(timeline, "total_duration_ms", 600000))

    if arrest_start is None or arrest_end is None or arrest_end <= arrest_start:
