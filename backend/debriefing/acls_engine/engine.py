...
_ALGORITHM_REGISTRY = {
    ...
    "megacode": {
        # Megacode needs all three rulesets — bundled as a dict
        "rules_file":        "cardiac_arrest_rules.json",  # primary (used as fallback key)
        "brady_rules_file":  "aha_2025_bradycardia_pulse_v2.json",
        "tachy_rules_file":  "aha_2025_tachyarrhythmia_pulse_v2.json",
...
class ACLSEngine:
    ...
    def evaluate(self, events_data: dict) -> List[dict]:
        ...
        if self._algo_type == "megacode":
            # Bundle all three rulesets so each sub-processor gets the correct one
            brady_rules_path = os.path.join(_ROOT, cfg["brady_rules_file"])
            tachy_rules_path = os.path.join(_ROOT, cfg["tachy_rules_file"])
            bundled_rules = {
                "cardiac_arrest": rules,
                "bradycardia":    _load_json(brady_rules_path),
                "tachyarrhythmia": _load_json(tachy_rules_path),
            }
            self._processor = cfg["processor"](bundled_rules, sub_type=self._sub_type)
        elif cfg["needs_subtype"]:
